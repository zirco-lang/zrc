...
pub fn type_expr_identifier<'input>(
    ...
    let ty_rc = scope.values.resolve_mut(i).ok_or_else(|| {
        let hints = if scope.types.has(i) {
            vec![HintKind::FoundTypeWithName(i.to_string()).in_span(expr_span)]
        } else {
            vec![]
        };
        ...
        Diagnostic(
            ...
            DiagnosticKind::UnableToResolveIdentifier(i.to_string()).in_span(expr_span),
            hints,
        )
    })?;
