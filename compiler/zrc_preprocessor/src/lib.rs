...
fn preprocess_internal(
    ...
    pragma_once_files: &mut HashSet<PathBuf>,
) -> Result<(), Diagnostic> {
            ...
            if directive == "pragma once" {
                ...
                let include_file = if let Some(path) = include_path.strip_prefix('"') {
                    path.strip_suffix('"')
                        .ok_or_else(|| {
                            DiagnosticKind::PreprocessorError(
                                "Unterminated include string".to_string(),
                            )
                            .error_in(Span::from_positions(0, line.len()))
                        })?
                        .to_string()
                    ...
                    path.strip_suffix('>')
                        .ok_or_else(|| {
                            DiagnosticKind::PreprocessorError(
                                "Unterminated include angle brackets".to_string(),
                            )
                            .error_in(Span::from_positions(0, line.len()))
                        })?
                        .to_string()
                    ...
                    return Err(DiagnosticKind::PreprocessorError(
                        "Include directive must use \"file\" or <file> syntax".to_string(),
                    )
                    .error_in(Span::from_positions(0, line.len())));
                };
                ...
                if pragma_once_files.contains(&canonical_path) {
                    chunk_start_line = line_num + 1;
                    continue;
                }
                ...
                let included_content = fs::read_to_string(&canonical_path).map_err(|err| {
                    DiagnosticKind::PreprocessorError(format!(
                        "Cannot read include file {include_file}: {err}"
                    ))
                    .error_in(Span::from_positions(0, line.len()))
                })?;
                ...
                let include_base = canonical_path.parent().ok_or_else(|| {
                    DiagnosticKind::PreprocessorError(format!(
                        "Cannot determine parent directory for {include_file}"
                    ))
                    .error_in(Span::from_positions(0, line.len()))
                })?;
    ...
    if has_pragma_once && let Ok(canonical) = base_path.join(file_name).canonicalize() {
        pragma_once_files.insert(canonical);
    }
