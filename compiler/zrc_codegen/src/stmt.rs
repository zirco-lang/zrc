                        ...
                        let place_opt = match output.expr.kind.value() {
                            ...
                            TypedExprKind::Index(ptr, idx) => Some(Place {
                                ...
                                kind: output
                                    ...
                                    .kind
                                    .span()
                                    .containing(PlaceKind::Index(ptr.clone(), idx.clone())),
                            }),
                            _ => None,
                        };
...
mod tests {
    ...
    mod inline_assembly {
        ...
        #[test]
        fn simple_inline_asm_generates_properly() {
            cg_snapshot_test!(indoc! {"
                fn test() {
                    // TEST: simple inline assembly with no operands
                    asm(\"nop\");
                }
            "});
        }
        ...
        #[test]
        fn inline_asm_with_inputs_generates_properly() {
            cg_snapshot_test!(indoc! {"
                fn test() {
                    let a: i32 = 10;
                    let b: i32 = 20;
                    // TEST: inline assembly with input operands
                    asm(\"nop\" : : \"r\"(a), \"r\"(b));
                }
            "});
        }
        ...
        #[test]
        fn inline_asm_with_single_output_generates_properly() {
            cg_snapshot_test!(indoc! {"
                fn test() {
                    let a: i32 = 10;
                    let b: i32 = 20;
                    let result: i32 = 0;
                    // TEST: inline assembly with single output operand
        ...
        #[test]
        fn inline_asm_with_multiple_outputs_generates_properly() {
            cg_snapshot_test!(indoc! {"
                fn test() {
                    let a: i32 = 10;
                    let b: i32 = 20;
        ...
        #[test]
        fn inline_asm_with_clobbers_generates_properly() {
            cg_snapshot_test!(indoc! {"
                fn test() {
                    let a: i32 = 10;
                    let result: i32 = 0;
        ...
        #[test]
        fn inline_asm_with_dereference_output_generates_properly() {
            cg_snapshot_test!(indoc! {"
                fn test() {
                    let value: i32 = 0;
                    let input: i32 = 42;
                    // TEST: inline assembly with variable as output
                    // (dereferences work but parser limitations)
                    asm(\"movl $1, $0\" : \"=r\"(value) : \"r\"(input));
                }
            "});
        }

        #[test]
        fn module_level_asm_generates_properly() {
            cg_snapshot_test!(indoc! {"
                // TEST: module-level assembly directive
                asm \"# Generated by Zirco Compiler\";
