...
// pub struct KeyTypeMapping<'input>(pub Spanned<Vec<Spanned<(Spanned<&'input
// str>, Type<'input>)>>>); impl Display for KeyTypeMapping<'_> {
...
// 	#[must_use]
// 	pub fn build_union_from_contents(span: Span, keys: KeyTypeMapping<'input>)
// -> Self { 		Self(TypeKind::Union(keys).in_span(span))
// 	}

// 	#[must_use]
// 	pub fn build_enum_from_contents(span: Span, keys: KeyTypeMapping<'input>) ->
// Self { 		Self(TypeKind::Enum(keys).in_span(span))
// 	}
...
// parentheses 		let test_cases = vec!["type x = (i32);", "type y = ((i32));",
// "type z = (*(i32));"];
