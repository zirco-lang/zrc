...
2. **Rust Stable Toolchain** - For building
   ```bash
...
**Standard build workflow (ALWAYS run in this exact order):**
...
# 3. Run nightly clippy (REQUIRED before committing - takes ~45-50 seconds)
cargo +nightly clippy --all-targets -- -D warnings

# 4. Run nightly fmt (REQUIRED before committing - takes <1 second)
...
- Debug builds go to `target/debug/zrc`, release builds to `target/release/zrc`
- Release build: `cargo build --release` (takes ~2-3 minutes, produces optimized binary)
